...
import ext.org.apache.poi.hhwpf.model.structure.section.control.gso.ControlRectangle;
import ext.org.apache.poi.hhwpf.model.structure.section.control.gso.shapecomponent.renderingnfo.RenderingInfo;
import ext.org.apache.poi.hhwpf.model.structure.section.control.gso.shapecomponent.renderingnfo.ScaleRotateMatrixPair;
 ...
 * @author neolord
 */
public abstract class ShapeComponent implements StreamWritable {
    private static final Logger logger = LoggerFactory.getLogger(ShapeComponent.class);
    /**
     * 개체 컨트롤 Id
     */
    private long gsoId;
    ...
    protected ShapeComponent(long gsoId) {
        this();
        this.gsoId = gsoId;
    }
    protected ShapeComponent(StreamReader sr, long gsoId) throws IOException {
        this.gsoId = gsoId;
        this.offsetX = sr.readSInt4();
        this.offsetY = sr.readSInt4();
        this.groupingCount = sr.readUInt2();
        this.localFileVersion = sr.readUInt2();
        this.widthAtCreate = sr.readSInt4();
        this.heightAtCreate = sr.readSInt4();
        this.widthAtCurrent = sr.readSInt4();
        this.heightAtCurrent = sr.readSInt4();
        this.property = new ShapeComponentProperty(sr.readUInt4());
        this.rotateAngle = sr.readUInt2();
        this.rotateXCenter = sr.readSInt4();
        this.rotateYCenter = sr.readSInt4();
        
        renderingInfo = new RenderingInfo(sr);
        logger.trace("++++++++++++ ShapeComponent commonPart has been completed ++++++++");
    }
    ...
    public void write(StreamWriter sw) throws IOException {
        ...
        sw.writeUInt2(this.getGroupingCount());
        sw.writeUInt2(this.getLocalFileVersion());
        sw.writeSInt4(this.getWidthAtCreate());
        sw.writeSInt4(this.getHeightAtCreate());
        sw.writeSInt4(this.getWidthAtCurrent());
        sw.writeSInt4(this.getHeightAtCurrent());
        sw.writeUInt4(this.getProperty().getValue());
        sw.writeUInt2(this.getRotateAngle());
        sw.writeSInt4(this.getRotateXCenter());
        sw.writeSInt4(this.getRotateYCenter());

        this.getRenderingInfo().write(sw);
    }
    ...
    public int getSize() {
        ...
        size += 2;
        size += 48;
        size += 48 * 2 * this.getRenderingInfo().getScaleRotateMatrixPairList()
                .size();
        return size;
    }
}
